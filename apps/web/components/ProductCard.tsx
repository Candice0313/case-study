...
function modelPageUrlFromName(name: string): string | null {
  ...
  if (!base || PS_PART.test(base)) return null;
  return PARTSELECT_MODELS + encodeURIComponent(base) + "/";
}
...
export function ProductCard({ card }: { card: ProductCardData }) {
  ...
  const isBaseUrl =
    ...
    rawUrl === (PARTSELECT_HOME + "/") ||
    rawUrl.replace(/\/$/, "") === PARTSELECT_HOME;
  const partNum = card.part_number?.trim();
  const partSearchUrl =
    partNum && PS_PART.test(partNum)
      ? PARTSELECT_HOME + "/Search.aspx?SearchTerm=" + encodeURIComponent(partNum)
      : null;
  const modelUrl = useMemo(
    () => (card.name && isBaseUrl && !partSearchUrl ? modelPageUrlFromName(card.name) : null),
    [card.name, isBaseUrl, partSearchUrl]
  );
  const href = (isBaseUrl ? partSearchUrl ?? modelUrl : null) ?? rawUrl ?? PARTSELECT_HOME + "/";
